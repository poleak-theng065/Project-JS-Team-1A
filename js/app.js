...
function handleButtonClick(quizContainer, card) {
  ...
  buttons.forEach((button) => {
    button.addEventListener("click", (event) => {
      const quizLevel = button.dataset.level; // Ensure you're getting the correct data attributes
      const quizSubject = button.dataset.subject;

      console.log("Quiz Level:", quizLevel);
      console.log("Quiz Subject:", quizSubject); // Debugging line

      if (quizContainer) quizContainer.style.display = "block"; // Show the quiz container
      if (categorySection) categorySection.style.display = "none";

      // Ensure proper initialization with the right path
